(function(){
  ...
  function applyFooterStatus(){
    ...
    // Priority 2: Dataset editor discovery/build status
    if(window.Hootsight && window.Hootsight.datasetEditorStatus) {
      const editorStatus = window.Hootsight.datasetEditorStatus;
      
      // Check discovery status
      if(editorStatus.discovery && editorStatus.discovery.status && editorStatus.discovery.status !== 'idle') {
        const percent = editorStatus.discovery.total_items > 0 
          ? Math.min(100, Math.round((editorStatus.discovery.processed_items / editorStatus.discovery.total_items) * 100))
          : 0;
        
        const projectName = editorStatus.discovery.project_name || '—';
        const processed = editorStatus.discovery.processed_items || 0;
        const total = editorStatus.discovery.total_items || 0;
        
        // Create HTML with inline progress bar
        const html = `Scanning ${projectName} <div class="footer-progress-inline"><div class="footer-progress-fill" style="width: ${percent}%"></div></div> ${percent}% ${processed}/${total}`;
        dom.footerStatus.html(html);
        dom.footerStatus.removeClass('error-text');
        console.debug('[footer] Discovery progress:', {percent, processed, total});
        return;
      }
      
      // Check build status
      if(editorStatus.build && editorStatus.build.status && editorStatus.build.status !== 'idle') {
        const percent = editorStatus.build.total_items > 0 
          ? Math.min(100, Math.round((editorStatus.build.completed_items / editorStatus.build.total_items) * 100))
          : 0;
        
        const projectName = editorStatus.build.project_name || '—';
        const completed = editorStatus.build.completed_items || 0;
        const total = editorStatus.build.total_items || 0;
        
        // Create HTML with inline progress bar
        const html = `Building ${projectName} <div class="footer-progress-inline"><div class="footer-progress-fill" style="width: ${percent}%"></div></div> ${percent}% ${completed}/${total}`;
        dom.footerStatus.html(html);
        dom.footerStatus.removeClass('error-text');
        console.debug('[footer] Build progress:', {percent, completed, total});
        return;
      }
    }
  ...
  const card = hs.createCard();
        ...
        const trainStopBtn = Q('<button type="button" class="primary">');
  ...
  async function loadProject(projectName){
    try {
      ...
      // Dataset tab removed from UI; don't auto-open dataset page.
  ...
  // Dataset page is removed from the UI; refreshDatasetInfo has been eliminated.
  ...
  ctx.fillStyle = getCssVar('--chart-fill-dark');
     ctx.fillRect(padding, padding, graphWidth, graphHeight);

    const gridLines = 4;
  ctx.strokeStyle = getCssVar('--chart-stroke-faint');
    ctx.lineWidth = 1;
    ctx.setLineDash([3, 5]);
  ...
  ctx.strokeStyle = getCssVar('--chart-stroke-weak');
    ctx.beginPath();
  ...
  const color = options.color || getCssVar('--graph-default');
      ...
      ctx.lineJoin = 'round';
      ctx.lineCap = 'round';
  // Canvas stroke glow disabled - keep shadowBlur 0 to prevent glows
  ctx.shadowBlur = 0;
      ctx.stroke();
      ctx.shadowBlur = 0;
  ...
  function renderTrainingStatus(){
    ...
    const graphs = [
  { key: 'train_epoch_accuracy', label: 'status_graph.train_accuracy', fallback: 'Train Accuracy', color: getCssVar('--graph-train-accuracy') },
  { key: 'val_epoch_accuracy', label: 'status_graph.val_accuracy', fallback: 'Validation Accuracy', color: getCssVar('--graph-val-accuracy') },
  { key: 'train_epoch_loss', label: 'status_graph.train_loss', fallback: 'Train Loss', color: getCssVar('--graph-train-loss') },
  { key: 'val_epoch_loss', label: 'status_graph.val_loss', fallback: 'Validation Loss', color: getCssVar('--graph-val-loss') }
    ];
