"""Swin Transformer module for Hootsight.

Provides comprehensive Swin Transformer model support including training, evaluation,
...
class SwinModel:
    """Swin Transformer model wrapper with training and evaluation capabilities."""

    def __init__(self, model_name: str = 'swin_t', num_classes: int = 10, pretrained: bool = True, task: str = 'classification'):
        """Initialize Swin model.

        Args:
            model_name: Name of Swin variant ('swin_t', 'swin_s', 'swin_b', 'swin_v2_t', 'swin_v2_s', 'swin_v2_b')
    ...
    def _create_model(self) -> nn.Module:
        """Create Swin model based on model_name and task."""
        if self.task in ('classification', 'multi_label'):
            weights_map = {
                ...
                'swin_v2_t': models.Swin_V2_T_Weights.DEFAULT,
                'swin_v2_s': models.Swin_V2_S_Weights.DEFAULT,
                'swin_v2_b': models.Swin_V2_B_Weights.DEFAULT
            }
            ...
            if self.model_name not in model_map:
                raise ValueError(f"Unsupported Swin variant: {self.model_name}")
            ...
            raise ValueError(f"Unsupported task: {self.task}. Swin currently supports 'classification' and 'multi_label'.")

        return model

    def get_optimizer(self, lr: float = 0.001, weight_decay: float = 1e-4) -> optim.Optimizer:
        """Get AdamW optimizer configured for Swin."""
    ...
    def get_scheduler(self, optimizer: optim.Optimizer, step_size: int = 7, gamma: float = 0.1) -> optim.lr_scheduler.StepLR:
        """Get StepLR scheduler configured for Swin."""
        # Try to get from config first
        try:
            training_config = SETTINGS['training']
        except Exception:
...
def create_swin_model(model_name: str = 'swin_t', num_classes: int = 10, pretrained: bool = True, task: str = 'classification') -> SwinModel:
    """Factory function to create Swin model."""
    return SwinModel(model_name, num_classes, pretrained, task)
