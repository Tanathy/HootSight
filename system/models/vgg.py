"""VGG module for Hootsight.

Provides VGG model support for classification and multi-label tasks.
"""
...
class VGGModel:
    """VGG model wrapper with training and evaluation capabilities."""

    def __init__(self, model_name: str = 'vgg16', num_classes: int = 10, pretrained: bool = True, task: str = 'classification'):
    ...
    def _create_model(self) -> nn.Module:
        if self.task not in ('classification', 'multi_label'):
            raise ValueError(f"VGG currently supports classification/multi_label tasks, got: {self.task}")

        weights_map = {
            'vgg11': models.VGG11_Weights.DEFAULT,
            'vgg11_bn': models.VGG11_BN_Weights.DEFAULT,
            'vgg13': models.VGG13_Weights.DEFAULT,
            'vgg13_bn': models.VGG13_BN_Weights.DEFAULT,
        ...
        if self.model_name not in model_map:
            raise ValueError(f"Unsupported VGG variant: {self.model_name}")
    ...
    def get_criterion(self) -> Optional[nn.Module]:
        """Get criterion based on config or task for VGG."""
        try:
            training_config = SETTINGS['training']
...
def create_vgg_model(model_name: str = 'vgg16', num_classes: int = 10, pretrained: bool = True, task: str = 'classification') -> VGGModel:
    return VGGModel(model_name, num_classes, pretrained, task)


def get_supported_vgg_variants() -> List[str]:
    return ['vgg11', 'vgg11_bn', 'vgg13', 'vgg13_bn', 'vgg16', 'vgg16_bn', 'vgg19', 'vgg19_bn']


def get_vgg_config(model_name: str) -> Dict[str, Any]:
    try:
        models_config = SETTINGS['models']['vgg']['variants']
    except Exception:
        raise ValueError("Missing required 'models.vgg.variants' in config/config.json")
    if model_name in models_config:
        return models_config[model_name]
    raise ValueError(f"Unsupported VGG variant (not found in config): {model_name}")
