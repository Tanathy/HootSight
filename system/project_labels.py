...
def persist_project_labels(project_name: str, labels: List[str], task: str,
                           base_override: Optional[str] = None) -> Optional[Path]:
    """Persist deterministic class labels for a project."""
    if not labels:
        warning(f"Skipping label persistence for {project_name}: labels list is empty")
        return None
...
def load_project_labels(project_name: str, base_override: Optional[str] = None) -> List[str]:
