(function(){
  ...
  function buildSectionEntityLayout(sectionKey, sectionSchema, current){
    ...
    Object.entries(defaultsSchema.properties).forEach(([entityKey, entitySchema]) => {
      ...
      const entityCard = Q('<div class="config-entity-card">');
  ...
  function buildConfigFieldCard(sectionKey, fieldKey, schema, current){
    ...
    const card = Q('<article class="config-field-card">');
  ...
  function buildAugmentationPhaseCard({ key: phase, titleKey, descriptionKey }){
    ...
    if(options.length){
      ...
      options.forEach(option => {
        ...
        const row = Q('<div class="augmentation-toggle-row">');
    ...
    const previewSection = Q('<div class="augmentation-preview-section">');
    ...
    if(previewDescription){
      previewSection.append(Q('<p class="augmentation-preview-description">').text(previewDescription));
    }
  ...
  function buildDatasetPage(){
    const wrap = Q('<div class="cards" id="dataset-cards">');
    setTimeout(() => actions.refreshDatasetInfo(), 0);
    return wrap.elements[0];
  }
